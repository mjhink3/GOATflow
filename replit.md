...
- **Gamification:** "Hay & Fresh Cheese" economy: Hay is earned per task completion (50 for Summit Call, 10 for others), with a speed bonus. 500 Hay automatically converts to 1 Fresh Cheese. "Cheese Churn Rate" (XP) dictates user level and "Ascension Rank." Custom icons (`static/icon_hay.png`, `static/icon_fresh_cheese.png`) are used throughout — all 🌾/🧀 emoji replaced in stat cards, sidebar stats, hay/cheese reward dialogs, pasture gauge bar, Goatifications toggle rows, and Tutorial Card 4 (including mascot callout footer).
...
- **Trail Notes (Part 1):** Completed Track entries in the Trail dialog show logged/completed dates, time elapsed, and Hay earned. Each entry has a client-side Trail Note (stored in `goatflow_trail_notes` localStorage) with inline add/edit UI injected via JS.
- **Adaptive Completion Prompt (Part 2):** After completing a track, a floating micro-prompt slides up with a context-aware reflection question (based on days to complete / track type). Auto-dismisses after 10s. Saves response as Trail Note to localStorage.
- **Difficulty Multiplier System (Part 3):** GPT-4o assigns a `category` field to each Signal. After 20+ completions, Hay earned = base × (category_avg_days / overall_avg_days), clamped 0.5–5.0. Displayed in the Hay popup. Category stored in `signals` and `operational_log` DB tables. New `logged_at` and `hay_earned` columns in `operational_log`.
- **Pattern Recognition (Part 4):** After every 10 completion history entries, JS analyzes avoidance patterns and triggers a monthly insight Goatification (after 30+ days of use). Patterns stored in `goatflow_patterns` localStorage.
- **Adaptive Achievements (Part 5):** 4 achievements (The Hard Way, Pattern Broken, Know Thyself, Zero Bull) tracked in `goatflow_achievements` localStorage. Displayed in Trail dialog Achievements section (locked/unlocked states).
- **Empty State (Part 6):** Replaced with branded empty state using GOATflow logo, "The pasture is clear." heading, and progressive copy hierarchy.
- **Churn Animation (Part 7):** Processing sequence adds "Cutting the bull." as a final line in green (#4ade80), bold.
